...
gulp.task('mocha', function () {
  return gulp.src('test/index.html').pipe(mochaPhantomJS({ reporter: 'list' }));
});

gulp.task('build-stylesheets', function () {
  return gulp.src('./gh-pages-assets/stylesheets/*.scss').pipe(sass({ includePaths: ['./gh-pages-assets/stylesheets'] })).pipe(gulp.dest('./gh-pages-assets'));
});

gulp.task('build-scripts', ['lint'], function () {
  gulp.src('./src/jquery.textarea_autosize.js').pipe(gulp.dest('./dist')).pipe(rename('jquery.textarea_autosize.min.js')).pipe(uglify()).pipe(size({ showFiles: true })).pipe(gulp.dest('./dist'));
});
...
gulp.task('npm', ['tag'], function (done) {
  childProcess.spawn('npm', ['publish'], { stdio: 'inherit' }).on('close', done);
});
